// Unified controller for all test sessions (baseline + NE/ML models).
// Based on NE_controller.c — identical sensors, PID, lap detection, startup sequence.
// Supports three command modes at runtime via the "command_mode" JSON field:
//   0 = binary commands   [forward, yaw_increase, yaw_decrease]  (baseline + ycommand models)
//   1 = continuous values [forward_desired, yaw_desired]         (ycontinuous models)
//   2 = motor velocities  [m1, m2, m3, m4]                      (ymotors — excluded from testing)
//
// Extended packet: 4116 bytes per frame (vs 4104 in NE_controller.c):
...
// Set TEST_MODE to skip the full 500-step hover stabilisation (16s per test).
...
// Extended send: 4096-byte image + 4-byte lap + 4-byte speed
//              + 4-byte pos_x + 4-byte pos_y + 4-byte pos_z = 4116 bytes total.
...
ne_commands_t parse_ne_json(const char *json_str) {
    ...
    if (fwd_ptr) { fwd_ptr += strlen("\"forward_desired\":"); while (*fwd_ptr == ' ') fwd_ptr++; cmd.forward_desired = (float)atof(fwd_ptr); }
    const char *yaw_ptr = strstr(json_str, "\"yaw_desired\":");
    if (yaw_ptr) { yaw_ptr += strlen("\"yaw_desired\":"); while (*yaw_ptr == ' ') yaw_ptr++; cmd.yaw_desired = (float)atof(yaw_ptr); }
    ...
    // Command mode (sent by receiver every step; -1 = no packet this frame)
    const char *mode_ptr = strstr(json_str, "\"command_mode\":");
    if (mode_ptr) { mode_ptr += strlen("\"command_mode\":"); while (*mode_ptr == ' ') mode_ptr++; cmd.command_mode = (int)atoi(mode_ptr); }

    // Scalar updates
    const char *spd_ptr = strstr(json_str, "\"new_speed_scalar\":");
    if (spd_ptr) { spd_ptr += strlen("\"new_speed_scalar\":"); while (*spd_ptr == ' ') spd_ptr++; cmd.new_speed_scalar = (float)atof(spd_ptr); }
    const char *ys_ptr = strstr(json_str, "\"new_yaw_scalar\":");
    if (ys_ptr) { ys_ptr += strlen("\"new_yaw_scalar\":"); while (*ys_ptr == ' ') ys_ptr++; cmd.new_yaw_scalar = (float)atof(ys_ptr); }
...
ne_commands_t check_for_commands() {
    ne_commands_t cmd = {0};
    cmd.command_mode = -1;  // sentinel: -1 = no packet received this frame (EAGAIN)
...
int init_socket_connection() {
    ...
    const char *port_env = getenv("WEBOTS_CONTROLLER_PORT");
...
int main(int argc, char **argv) {
    ...
    // 2-second stabilisation wait (identical to all existing controllers)
    ...
    while (wb_robot_step(timestep) != -1) {
        ...
        // GPS position for packet (cast to float for transmission)
        float pos_x = (float)x_global;
        float pos_y = (float)y_global;
        float pos_z = (float)actual_state.altitude;
        ...
        // First image after startup stabilisation (fires once).
        // Normal: 500 steps (~16s).  TEST_MODE: 50 steps (~1.6s).
#ifdef TEST_MODE
        static int startup_counter = 0;
        startup_counter++;
        if (startup_counter == 50) {
            send_2d_array(camera, lap_count, speed_scalar, pos_x, pos_y, pos_z);
        }
#else
        static int startup_counter = 0;
        startup_counter++;
        ...
        // Update runtime mode whenever a real packet arrived.
        ...
        // ── MODE 0: Binary commands (baseline + ycommand models) ──────────────
        if (current_mode == 0) {
            if (got_packet) {
        ...
        // ── MODE 2: Motor velocities (ymotors — not used in current test plan) ─
